...
Q5) 지금까지 살펴본 이진 검색 알고리즘 프로그램은 검색할 값이 하나 이상일 경우 맨 앞의 요소를 찾지 못한다. 
	맨 앞의 요소를 찾는 binSearchX 메서드를 작성해보세요.
	int[] a = {1, 3, 5, 7, 7, 7, 7, 8, 8, 9, 9};
	ex) static int binSearchX(int[] a, int n, int key)
	...
	% Java는 배열에서 이진 검색을 하는 표준 라이브러리를 제공한다 - java.util.Arrays.binarySearch
	이를 사용함으로써 직접 코딩의 필요를 줄이고, 모든 자료형 배열에서 검색할 수 있다.
	
	
##### 보충수업 - 클래스 메서드와 인스턴스 메서드
	1) 인스턴스 메서드(비정적 메서드) - static이 붙지 않았다.
	2) 클래스 메서드(정적 메서드) - static이 붙었다.			 
